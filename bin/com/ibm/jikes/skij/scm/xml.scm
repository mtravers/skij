; playing with TRL XML parser
; (see http://monet.trl.ibm.com/webcasting/xmlprocessor/tutorial/)

; todo:
; trim strings in tree display (done, but maybe eliminate entirely)
; read from network
; interface for writing new XML
; DCD generator
...
'(define doc (parse-xml-file '"d:/xml/trlxml/Hamlet.xml"))'
'(define doc (parse-xml-file '"d:/xml/osd-ex.xml")) 
...
(define (get-attribute xml att-name)
  (%or-null (invoke xml 'getAttribute (string att-name)) #f))
...
;;; turn callup data into XML document
  
'(define kahuna (car (get-employees-by-name '"Gerstner, Lou")))

'(define (employee-known-subordinates mgr)
  (define result '())
  (map-hashtable (lambda (key emp)
		   (if (and (eq? mgr (employee-manager emp))
			    (not (eq? mgr emp)))
		       (push emp result)))
...
'(make-xml-doc kahuna
	      employee-known-subordinates
	      (lambda (emp) 
		(new 'com.ibm.xml.parser.TXElement (employee-name emp))))

(define (tree-demo . file)
  (if (null? file)
      (set! file '"d:/xml/osd-ex.xml")
      (set! file (car file)))
