...
(define (output-method-form method object class)
  ...
  (with-env
;   'form `((action ,(if constructor? 'create 'invoke)))
   'form (list (list 'action (if constructor? 'create 'invoke)))
   (lambda ()
     ...
     (lenv (table (align bleedright))
	   (lenv tr			;first row
		 ...
		 (env td
			  ...
			  (env i
			       (with-env 'a `((href ,(method-doc-url method args))
					      (target "doc"))
...
;  (ptrace collect)
  ...
  (bypassing-security
   (let ((collect collect))		;this is looked up dynamically, bypassing security runs in different dynamic context, so rebind here.
     (process-path-entry filename)))
  (set! tree
...
(define-command static-methods (class)
  ...
  (wsdoc (lambda ()
	 ...
	 (define methods (filter member-static? ;part of inspect
				 (vector->list (invoke class 'getMethods))))
	 (define static-class-name `(class ,(invoke class 'getName)))
	 (for-each 
	  (lambda (method)
	    (output-method-form method static-class-name class))
