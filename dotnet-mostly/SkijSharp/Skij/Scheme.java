...
public class Scheme {
  ...
  public static void start(String initString, boolean threadp) {
    ...
    // for starting up from lame jscript listener...
    public static void startConsole() {
	start("(make-awt-listener-window)", false);
    }
...
//   }
    ...
    // version for C#
    // This needs to be figured out at runtime, if anybody but me starts to use it (unlikely).
    static String skijBase = "d:/mt/projects/skij/SkijSharp/Skij/";

    // nice if this worked, but it appears not to, and haven't had time to look into it.
    //    static String skijBase = System.Reflection.Assembly.GetExecutingAssembly().get_CodeBase();

    
    public static void loadResource(String name, String from) throws SchemeException, ClassNotFoundException, IOException {
	File file = new File(skijBase + name);
	InputStream s = new FileInputStream(file);
	if (s == null)
	    throw new SchemeException("resource " + name + " not found: " + file);
	else {
      if (!quiet) System.out.println("Loading resource " + name + "...");
      new SchemeListener(s).repl();
      procedure("provide-resource").safeApply(Cons.list(name));
